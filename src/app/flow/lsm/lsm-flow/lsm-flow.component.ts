...
@Component({
  selector: 'app-lsm-flow',
  templateUrl: './lsm-flow.component.html',
  styleUrls: ['./lsm-flow.component.scss']
})
export class LsmFlowComponent {
  ...
  goToLsmContent(): void {
    if (this.role) {
      ...
      this.router.navigate(['/lsmContent'], { queryParams: { role: this.role } });
    } else {
      // Navigate to pmsContent page without role parameter if role is not stored
      this.router.navigate(['/lsmContent']);
    }
  }
  toggleDetailedView() {
